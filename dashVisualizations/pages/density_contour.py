...
layout = html.Div(
    [
        html.Div(id="densityContour-container", children=[]),
    ]
)
# @callback(
...
@callback(
    Output("densityContour-container", "children"),
          [
          State("densityContour-container",'children'),
    ...
    for i,c in enumerate(cols):
        for j,c2 in enumerate(cols):
            if j>=i:
                continue
            try:
                fig = px.density_contour(df, x=c, y=c2, marginal_x="histogram", marginal_y="histogram")
                el = html.Div(
                    [
